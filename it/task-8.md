...
_Explanation of the Example. In the first case we have sequence starting with `5` and increasing by `2` each time.
We want to sum `3` elements from it `5 + 7 + 9 = 21`. The second is easier. It starts with `3` but increment is `0`,
so it is `3 + 3 + ... + 3 = 30` (total of `10` elements)._
